...
- [x] **`game.js`** — `startGame()`: 新增 `this.dealerStreak = 0` 初始化
- [x] **`game.js`** — `startHand()`: 莊家胡牌→`dealerStreak++`、過莊→`dealerStreak=0`
- [x] **`game.js`** — `play()` 流局處理: 檢查莊家 `tilesNeeded().waiting`，有聽→連莊
- [x] **`game.js`** — `getAllClaims()`: 收集所有 `CLAIM.WIN`，多家胡→`{multiple:true, winners:[]}`
- [x] **`game.js`** — 新增 `processMultiWin()` (55 行): 多家胡牌結算，放槍者分別付每位贏家
- [x] **`taiwan-classical.js`** — `checkWinnerHandPatterns()`: 連莊加台（連N拉N）
- [x] 圈風系統：原版 `windOfTheRound` 已正常運作 ✅
- [x] 遊戲結束判定：`windOfTheRound===4` 結束 ✅
- [x] 瀏覽器驗證：22 手無 JS 錯誤 ✅、連莊加台正確 ✅
- [x] Git commit: `e097a86`

**備註：** 圈風追蹤、遊戲結束判定、莊家胡牌不過莊（`pass_on_east_win=false`）在原版已實作。
...
- Phase 4 已完成（全面中文化、UI版面調整、加入語音提示）
- Phase 5 持續進行中，手動測試及 UI 打磨已達標，可視情況補充 UnitTest
