...
class Pattern {
  ...
  recurse(seen, chain, to_remove, results, singles, pairs, sets) {
    ...
    // REQUIRED_SETS sets and a pair is totally a winning path.
    if (sets.length === REQUIRED_SETS && pairs.length === 1 && singles.length === 0) {
    ...
    // REQUIRED_SETS sets and a single is one tile away from winning.
    else if (sets.length === REQUIRED_SETS && pairs.length === 0 && singles.length === 1) {
      this.markWin(results, singles[0], Constants.PAIR);
    }

    // (REQUIRED_SETS-1) sets and two pairs are one tile away from winning.
    else if (sets.length === (REQUIRED_SETS - 1) && pairs.length === 2) {
      this.markWin(results, pairs[0], Constants.PUNG);
      this.markWin(results, pairs[1], Constants.PUNG);
    }

    // (REQUIRED_SETS-1) sets, a pair, and two singles MIGHT be one tile away from winning.
    else if (sets.length === (REQUIRED_SETS - 1) && pairs.length === 1 && singles.length === 2) {
